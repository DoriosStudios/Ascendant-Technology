...
const DIG_COMPONENT_ID = "utilitycraft:dig_pebble";
...
function handleDigPebbleUseOn({ block, source, itemStack }, params = {}) {
    ...
    if (itemStack.durability?.damage?.(durabilityCost, durabilityChance)) {
        ...
        source.setEquipment("Mainhand", undefined);
        source.playSound("random.break");
    }
}

const digPebbleUseOnEvent = world.beforeEvents?.itemUseOn ?? world.afterEvents?.itemUseOn;

if (digPebbleUseOnEvent?.subscribe) {
    digPebbleUseOnEvent.subscribe(event => {
        const params = getCustomComponentParams(event.itemStack, DIG_COMPONENT_ID);
        if (!params) return;

        handleDigPebbleUseOn(event, params);
    });
} else {
    console.warn("[Ascendant Technology] itemUseOn event is unavailable; dig pebble component is disabled.");
}
