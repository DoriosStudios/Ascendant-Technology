 ...
 * @property {Partial<Record<ContainerFace,string>>} initialModes Modes applied only when the entity has no persisted item policy.
 */
...
/**
 * @param {unknown} value
 * @param {ItemIOMode[]} modes
 * @param {string} path
 * @returns {Partial<Record<ContainerFace,string>>}
 */
function normalizeInitialModes(value, modes, path) {
  ...
  const operationalIds = new Set(modes
    .filter((mode) => mode.id !== DEFAULT_ITEM_IO_MODE && mode.id !== DISABLED_ITEM_IO_MODE)
    .map((mode) => mode.id));
  /** @type {Partial<Record<ContainerFace,string>>} */
  const initialModes = {};
  for (const direction of DIRECTIONS) {
    const modeId = raw[direction];
    ...
    initialModes[/** @type {ContainerFace} */ (direction)] = modeId;
  }
  return initialModes;
}
...
function createEmptyConfig(definition) {
  ...
  for (const direction of DIRECTIONS) {
    ...
    if (!mode) continue;
    if (mode.inputSlots.length > 0) config.inputConfig[face] = [...mode.inputSlots];
    if (mode.outputSlots.length > 0) config.outputConfig[face] = [...mode.outputSlots];
  }
  return config;
}
