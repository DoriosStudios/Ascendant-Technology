...
const SHOVEL_COMPONENT_ID = "utilitycraft:shovel";
 ...
 */
const shovelUseOnEvent = world.beforeEvents?.itemUseOn ?? world.afterEvents?.itemUseOn;

if (shovelUseOnEvent?.subscribe) {
    shovelUseOnEvent.subscribe(event => {
        const params = getCustomComponentParams(event.itemStack, SHOVEL_COMPONENT_ID);
        if (!params) return;
    ...
    console.warn("[Ascendant Technology] itemUseOn event is unavailable; shovel component is disabled.");
}
